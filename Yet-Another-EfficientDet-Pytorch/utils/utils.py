...
def aspectaware_resize_padding(image, width, height, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
    old_h, old_w, c = image.shape

    # 保证new_w:new_h=old_w:new_w
    if old_w > old_h:
        new_w = width
    ...
    image = nn.functional.pad(image, (0, padding_w, 0, padding_h), "constant", 0)
...
def preprocess_video(*samples, width, height,mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229), use_video=True):
    if use_video:
        images = samples
        image_origin = [img.copy() for img in samples]
    else:
        images = [cv2.imread(img_path) for img_path in samples]

        image_origin = [image.copy() for image in images]

    t0 = time.time()
    imgs_meta = [aspectaware_resize_padding(image, width, height, mean, std) for image in images]
    framed_imgs = [img_meta[0] for img_meta in imgs_meta]
    framed_metas = [img_meta[1:] for img_meta in imgs_meta]
    return image_origin, framed_imgs, framed_metas, t0
